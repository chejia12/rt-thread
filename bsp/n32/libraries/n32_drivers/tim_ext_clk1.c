...
static rt_size_t n32_tim_ext_counter_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    struct n32_tim_ext_counter_device *device = (struct n32_tim_ext_counter_device *)dev;
...
#include <rtdbg.h>
#define DEV_NAME "pulse8"

static rt_device_t pwm_input_dev = RT_NULL;
static rt_uint32_t total_pulse_count = 0;
...
static int pwm_input_test(void)
{
    ...
    rt_time_t timer = rt_timer_create("pulse", timeout_1S, pwm_input_dev, 1000, RT_TIMER_FLAG_PERIODIC | RT_TIMER_FLAG_HARD_TIMER);
